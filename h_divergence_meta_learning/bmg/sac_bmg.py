...
class Agent:
    ...
    def rollout(self):
        ...
        for idx in range(self.rollout_steps_per_call):
            ...
            self.obs_replay_buffer[self.effect_num] = obs
            bootstrap_states[idx] = obs
            self.next_obs_replay_buffer[self.effect_num] = obs_
            self.reward_replay_buffer[self.effect_num] = reward
            self.action_replay_buffer[self.effect_num] = action
    ...
    def train_step(self, epoch, meta_alpha, bootstrap=False):
        idx = np.random.choice(self.effect_num + 1, self.train_batch_size, replace=False)
