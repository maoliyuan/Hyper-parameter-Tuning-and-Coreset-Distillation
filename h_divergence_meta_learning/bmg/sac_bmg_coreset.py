...
class Policy(nn.Module):
    def __init__(self, input_dims, n_actions, lr, fc1_dims=256, fc2_dims=256, cuda_device_num = 0, gamma=0.99):
...
class Agent:
    def __init__(self, input_dims, n_actions, gamma, prior, lr, m_lr, alpha, betas, eps, name, 
                    env, steps, K_steps, L_steps, rollout_steps_per_call, buffer_size, coreset_size, min_size_before_training, train_batch_size, discount, meta_start_epoch, cuda_device_num, random_seed):
        super(Agent, self).__init__()
        self.device = T.device('cuda:'+str(cuda_device_num) if T.cuda.is_available() else 'cpu')

        self.policy = Policy(input_dims, n_actions, lr, fc1_dims = 256, fc2_dims = 256, cuda_device_num = cuda_device_num)
        self.policy_k = Policy(input_dims, n_actions, lr, fc1_dims = 256, fc2_dims = 256, cuda_device_num = cuda_device_num)
    ...
    def rollout(self):
        ...
        for idx in range(self.rollout_steps_per_call):
            ...
            if self.effect_ptr == self.buffer_size:
    ...
    def coreset_compress(self):
        ...
        unit_size = 5000
        unit_select = int(unit_size * (self.coreset_size / self.effect_ptr))
        unit_round = (self.effect_ptr + unit_size - 1) // unit_size
        
        order = np.array([])
        for round in range(unit_round):
            min_idx = unit_size * round
            max_idx = min(unit_size * (round + 1), self.effect_ptr)
        ...
        select_actions = self.action_replay_buffer[order]
        select_rewards = self.reward_replay_buffer[order]
        select_observations = self.obs_replay_buffer[order]
        select_next_obs = self.next_obs_replay_buffer[order]
        ...
        self.obs_replay_buffer[:self.coreset_size] = select_observations
        self.next_obs_replay_buffer[:self.coreset_size] = select_next_obs
        self.action_replay_buffer[:self.coreset_size] = select_actions
        self.reward_replay_buffer[:self.coreset_size] = select_rewards
        self.effect_ptr = self.coreset_size
        
        print(f"compress success! size: {self.effect_ptr}")
    ...
    def train_step(self, epoch, meta_alpha, bootstrap=False):
        ...
        Policy Loss
        """
        qf1_copy = Qfunc(input_dims, self.n_actions, lr, fc1_dims = 256, fc2_dims = 256, cuda_device_num = cuda_device_num).to(self.device)
        qf2_copy = Qfunc(input_dims, self.n_actions, lr, fc1_dims = 256, fc2_dims = 256, cuda_device_num = cuda_device_num).to(self.device)
        ...
        if ~bootstrap and epoch >= self.meta_start_epoch:
            policy_loss = -torch.mean(meta_alpha * action_dist.entropy() + (action_dist.probs * q_new_actions).sum(dim=-1))
        else:
            policy_loss = -torch.mean(self.alpha * action_dist.entropy() + (action_dist.probs * q_new_actions).sum(dim=-1))
    ...
    def plot_results(self):
        ...
        plt.savefig('res_1/cumulative_reward')
        plt.close(cr)
        ...
        plt.savefig('res_1/entropy_rate')
        plt.close(er)
...
if __name__ == "__main__":
    ...
    agent = Agent(input_dims, n_actions, gamma, prior, lr, m_lr, alpha, betas, eps, name, env, 
                    steps, K_steps, L_steps, rollout_steps_per_call, buffer_size, coreset_size, min_size_before_training, 
